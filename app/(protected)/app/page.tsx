...
export default function NewRedesignPage() {
  ...
  const ensureDraftRoom = async () => {
    ...
    try {
      ...
      // Create draft row with safe defaults
      const payload = {
        user_id: user.id,
        room_type: roomType,
  ...
  // Persist as user selects values (update existing room row)
  useEffect(() => {
    if (!roomId) return;
  ...
  const onGenerate = async () => {
    ...
    setBusy(true);
    try {
      // Save latest selections + mark generating so UI is consistent
      ...
      // If spam guard trips, route to result anyway (it’s generating already)
      if (res.status === 409) {
              ...
              <div className="space-y-5">
                ...
                {busy && (
                  <div className="text-xs text-[#6A6A6A]">
                    Working… you’ll be redirected to the live progress screen.
                  </div>
                )}
              </div>
            )}
