...
export default function NewRedesignPage() {
  ...
  async function ensureDraftRoom() {
    ...
    try {
      ...
      const { data, error: insertErr } = await supabase.from("rooms").insert(payload).select("id").single();
  ...
  // Persist selections once draft exists
  useEffect(() => {
    if (!roomId) return;
  ...
  const onGenerate = async () => {
    ...
    if (!inputImagePath) return setError("Please upload a photo first.");
    if (!goal || !styleKey || !budgetTier) return setError("Please complete all steps before generating.");
    ...
    try {
      ...
      if (!id) {
        ...
        if (!id) throw new Error("room_init_failed");
      }

      // Persist final selections; keep draft (server will lock to generating)
      await patchRoom(id, {
        room_type: roomType,
        goal,
        style_key: styleKey,
          ...
          <div className="mt-6">
            ...
            {step === "style" && (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {STYLES.map((s) => (
                  <StyleTile key={s} styleKey={s} selected={styleKey === s} onSelect={() => setStyleKey(s)} />
                ))}
              </div>
            )}
